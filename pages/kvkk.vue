...
const globalStore = useGlobalStore(),
    ...
    kvkkContent = computed(() => contentStore.contentData?.data),
    breadCrumbs = computed(() => contentStore.contentData?.breadcrumbs)

await useAsyncData(() => contentStore.content('kvkk'))
...
<template>
  <v-container fluid="">
    <block-title title="Kişisel Verilerin Korunması Kanunu"/>
    ...
    <v-row class="main-row">
      ...
      <v-col md="9" cols="12">
        <v-card class="corporate-content">
          <v-card-title>{{ kvkkContent.title }}</v-card-title>
          <v-card-text v-html="kvkkContent.content"></v-card-text>
        </v-card>
      </v-col>
    </v-row>
